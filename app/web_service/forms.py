...
class LimitForm(FlaskForm):
	...
	def validate(self):
		if not Form.validate(self):
			return False
		return self.delay.data >= 0 and self.loTemp.data < self.hiTemp.data and self.loSpeed.data < self.hiSpeed.data
...
class GraphForm(FlaskForm):
	...
	def update_choices(self):
		...
		self.sensorList.choices = [(x, names[x]) for x in db.get_table_name()]
